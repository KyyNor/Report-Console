...
- **文档（meta/）**：需求/设计 `.md` / `.txt` / `.html` 与过程创建语句 `.sql` 默认在项目 meta/ 目录；可通过工作台导入文本型文档并自动复制到这里，Agent 可读写作为上下文。HTML 只作为文本存储与传递；工作台的 HTML 预览仅经 sandbox iframe（`sandbox=""`，禁脚本、禁同源），内容不在应用上下文执行。过程定义可按清单使用其他项目内相对路径。
...
## Agent 模型 Provider 档案（参考 Prime Agent）
